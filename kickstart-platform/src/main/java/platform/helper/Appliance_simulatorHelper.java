...
public class Appliance_simulatorHelper extends BaseHelper {
	...
	public BaseResource[] getCustomerVehicle(String customerId) {
		return Appliance_simulatorHelper.getInstance().getByCustomerIdForType(customerId,"VEHICLE");
	...
	public BaseResource getApplianceLocation(String applianceId) {
		return Appliance_simulatorHelper.getInstance().getSelectedFieldsById(applianceId, new String[]{appliance.FIELD_ID,appliance.FIELD_LATITUDE,
	...
	public BaseResource[] getSchoolBusAdminDetail(String customerId, String[] schoolIds) {
		...
		if (!Util.isEmpty(schoolIds)) {
			appliances = Appliance_simulatorHelper.getInstance().getSchoolVehicle(schoolIds);
		} else {
			appliances = Appliance_simulatorHelper.getInstance().getByCustomerIdForType(customerId,"VEHICLE");
		}
		if (Util.isEmpty(appliances)) 
			return null;
		for(BaseResource resource : appliances) {
	...
	public void updateLastDataUpdateTime(String applianceId, Date time) {
		...
		try {
			Appliance_simulatorHelper.getInstance().update(_appliance);
	...
	public void updateHAPowerDeviceData(controller _controller,
		...
		try {
			Appliance_simulatorHelper.getInstance().update(__appliance);
		} catch (ApplicationException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
