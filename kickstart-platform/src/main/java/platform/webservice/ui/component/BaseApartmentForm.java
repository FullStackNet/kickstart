...
public abstract class BaseApartmentForm extends BaseView {
	...
	public BaseApartmentForm(UIServletContext ctx) {
		super();
		mContext = ctx;
	...
	void buildUI(String op, Map<String, Object> data) {
		...
		mForm.addAttribute("class","flat_form form-horizontal");
		getView().addChild(mForm);
		Div _formPanel = new Div(null,"panel-group position-relative");
		Div formPanel = new Div(null,"panel panel-default");
		_formPanel.addChild(formPanel);
		Div _formBody = new Div(null, "panel-collapse collapse in");
		formPanel.addChild(_formBody);
		Div formBody = new Div(null, "panel-body text-gray-black ");
		
		_formBody.addChild(formBody);
		
		mForm.addChild(_formPanel);
		...
		for (int i = 0; i < mDefinition.getBlocks().size(); i++) {
			...
			if (mDefinition.getWidth() == 100) {
				blockdiv.addAttribute("style", ""+"width:"+mDefinition.getWidth()+"%");					
			} else {
			...
			for(int j=0; j < block.getFields().size(); j++) {
				...
				if (field.getCompomentType() == UIConstants.COMPONENT_TYPE_TEXTINPUT) {
					...
					textEdit.addAttribute("class","form-control");
					...
					password.addAttribute("class","form-control");
					if (value != null) {
					...
					if ((list != null) && (list.size() > beforeSize)  && field.isHideOnSingleEntry() && (actualSize == 1)) {
						...
						COMBO combo = new COMBO(field.getName(), "form-control",field.getDatasource().getData(),value);
						if (block.isHorizontalDisplay()) {
							if (field.getWidth() > 0) {
		...
		if (mDefinition.getButtonList().size() > 0) {
			...
			if (mDefinition.getWidth() == 100) {
				formFooterDiv.addAttribute("style", ""+"width:"+mDefinition.getWidth()+"%");
			} else {
				formFooterDiv.addAttribute("style", ""+"width:"+mDefinition.getWidth()+"px");
			}
			formFooterDiv.addAttribute(new Attribute("align","right"));
			if (mDefinition.isSaveandproceed()) {
				BUTTON button = new BUTTON("save","margin-t-10 btn btn-info btn-block text-bold" ,true);
				button.setValue("Save and Proceed");
				mForm.addChild(button);
			} else {
