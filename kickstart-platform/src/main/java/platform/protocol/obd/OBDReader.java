...
public class OBDReader extends Reader {
	char startchar;
	public OBDReader(char startChar) {
		super();
	...
	public Message data2Message(Session session,MessageManager messageManager, String data) {
		...
		int command = Integer.parseInt(commandStr.substring(2, 4), 16);
		int category =  Integer.parseInt(commandStr.substring(0, 2), 16);
		index++;
	...
	public Message read(Session session, Object _handle, MessageManager messageManager) throws ApplicationException {
		...
		while (true) {
			...
			data = data.append(ch);
			dataLength++;
			if (dataLength >= OBDProtocolProvider.MAX_MESSAGE_LENGTH) {
