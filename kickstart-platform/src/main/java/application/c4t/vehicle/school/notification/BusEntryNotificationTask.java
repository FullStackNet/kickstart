...
public class BusEntryNotificationTask extends NotificationTask {
	public BusEntryNotificationTask() {
		super(NotificationFactory.NOTIFICATION_BUS_ENTRY);
	...
	void sendSMS2Users(notification _notification, 
		...
		for(Map.Entry<String, student> entry : smsAlertMap.entrySet()) {
			...
			smsMessage.setType(ApplicationConstants.SMS_TYPE_SEND_BUS_ENTRY);
	...
	void sendNotification2Users(notification _notification, Map<String, BaseResource> userMap,
			String appAlert,
			String student_name,String type,String customerId,student _student) {
		for(Map.Entry<String, BaseResource> entry : userMap.entrySet()) {
			...
			if ("Y".equals(appAlert) && (_user.getMobile_no() != null)) {
				...
				notificationMessage.setTitle("BUS ATTENDANCE MESSAGE FROM "+student_name.toUpperCase()+ " SCHOOL ");
				notificationMessage.setType(ApplicationConstants.NOTIFICATION_TYPE_SEND_BUS_ENTRY);
				Map<String, String> map = new HashMap<String, String>();
				map.put(NotificationFactory.NOTIFICATION_DATA_PARAMETER_TYPE, type);
