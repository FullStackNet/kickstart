...
public class SchoolExitNotificationTask extends NotificationTask {
	public SchoolExitNotificationTask() {
		super(NotificationFactory.NOTIFICATION_SCHOOL_EXIT);
	...
	void sendSMS2Users(notification _notification, 
		...
		for(Map.Entry<String, student> entry : smsAlertMap.entrySet()) {
			...
			smsMessage.setType(ApplicationConstants.SMS_TYPE_SEND_SCHOOL_EXIT);
	...
	void sendNotification2Users(notification _notification, Map<String, BaseResource> userMap,
		...
		for(Map.Entry<String, BaseResource> entry : userMap.entrySet()) {
			...
			if ("Y".equals(appAlert) && (_user.getMobile_no() != null)) {
				...
				notificationMessage.setType(ApplicationConstants.NOTIFICATION_TYPE_SEND_SCHOOL_EXIT);
				Map<String, String> map = new HashMap<String, String>();
	...
	void sendNotification(notification _notification, 
		...
		if (!"Y".equals(_school.getFeature_school_exit_sms())) {
			smsAlert = "N";
		}
